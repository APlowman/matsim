        ...
        * Validate point_defects
        ...
        for pd in point_defects:
            ...
            if pd.defect_species.lower() != 'v' and pd.host_species != 'i':
                ...
                species_idx[pd.index] = np.where(
                    species == pd.defect_species)[0][0]
                species_count_idx[pd.index] = np.where(
                    species_count == -9999)[0][0]
                if 'bulk_coord_number' in self.atom_labels:
                    bulk_coord_num_idx[index] = np.where(
                        bulk_coord_num == -9999)[0][0]
                if 'charge' in self.atom_labels:
            ...
            if pd.host_species.lower() == 'i':
                # Atoms
                self.atom_sites = np.append(
                    self.atom_sites, self.interstice_sites[:, pd.index][:, np.newaxis], axis=1)
                if pd.defect_species not in species:
                    species = np.append(species, pd.defect_species)
                species_idx = np.append(species_idx, np.where(
                    species == pd.defect_species)[0][0])
                species_count_idx = np.append(
                    species_count_idx, np.where(species_count == -9999)[0][0])
                crystal_idx = np.append(
                    crystal_idx, interstice_crys_idx[pd.index])
                if 'bulk_coord_number' in self.atom_labels:
                    bulk_coord_num_idx = np.append(
                        bulk_coord_num_idx, np.where(bulk_coord_num == -9999)[0][0])
                if 'charge' in self.atom_labels:
                    rel_charge = pd.charge
                    abs_charge = 0 + rel_charge
                    if abs_charge not in charge:
                        charge = np.append(charge, abs_charge)
                    charge_idx = np.append(
                        charge_idx, np.where(charge == abs_charge)[0][0])

                # Interstices
                interstice_occ_idx[pd.index] = np.where(
                    interstice_occ == True)[0][0]
            ...
            if pd.defect_species.lower() == 'v':
                ...
                if 'bulk_coord_number' in self.atom_labels:
                    bulk_coord_num_idx = np.delete(
                        bulk_coord_num_idx, pd.index)
                if 'charge' in self.atom_labels:
